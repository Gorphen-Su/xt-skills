...
4. **完全隔离**：不修改原封装层的任何逻辑
5. **升级无关**：命令逻辑基于 openspec 原始文件，升级不受影响
...
```
.clauude/skills/xt-openspec-enhance/
├── SKILL.md                    # 本文件 - 定义增强规则
└── scripts/
    ├── git_utils.py            # 共享工具函数
...
| 修改 | 保持原样不变 | 独立新增 |
| 命令文件 | `.claude/commands/opsx/*.md` | **不修改** |
| 关系 | 无依赖、无调用、无交互 | 独立运行 |
...
1. **与 openspec 升级无关**
   - 不修改 `commands/opsx/*.md` 文件
   - openspec 升级时不会覆盖自定义逻辑

2. **完全隔离**
   - `xt-openspec-wrapper` 完全不受影响
   - 两个 skill 独立运行
